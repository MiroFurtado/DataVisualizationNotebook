...
AUTHOR: Miro Furtado
TODO:
Make dots correspond to actual districts.
Overlay congressional, state legislature so that we can get a better picture of money distribution. 
...
class State:
    ...
    def gen_report(self,states=None):
        ...
        if(states or len(self.districts) <= 15):
            self.regr = gen_full_pvi_scatter(states)
        else:
    ...
    def gen_pvi_fundraising_scatter(self):
        ...
        print(plt.plot(fig, include_plotlyjs=False, output_type='div', config={'displayModeBar': False}, show_link=False))    
    ...
    def gen_stack_graph(self,out_file=None):
        ...
        fig = go.Figure(data=data, layout=layout)
        #pio.write_image(fig, 'stack_graph.png',scale=4)
        plt.iplot(fig, filename=out_file,show_link=False, config={'displayModeBar': False})
        print(plt.plot(fig, include_plotlyjs=False, output_type='div', config={'displayModeBar': False}, show_link=False))
